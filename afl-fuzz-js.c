...
//  printf("target_path=%s",target_path);
 ...
 /*
  kill_signal = WTERMSIG(status);
   if (kill_signal == 0)
{
	printf("kill_signal=%d",kill_signal);
	return FAULT_CRASH;//V8 DEBUG CHECK
}
*/
  if (WIFSIGNALED(status) && !stop_soon) {

    kill_signal = WTERMSIG(status);
    if (kill_signal ==SIGILL) return FAULT_CRASH;//V8 DEBUG CHECK
...
#define SP20    SP10 SP10
  ...
  if (dumb_mode) {
    ...
    if (cycles_wo_finds < 25 || min_wo_finds < 30){ strcpy(tmp, cYEL); flag = 0;}else
...
/* Check if dharma installed in your commputer. */
void check_dharma()
{
  int stderr_fd[2];
  const char *cmd="dharma";
  const char *argv[]={cmd};
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Q:g:")) > 0)
	...
	if (dg_dir) FATAL("Multiple -g options not supported");
        dg_dir = optarg;

	//FATAL("dg_dir=%s",optarg);
	break;
